...
$ConfiguredPublicKey = "<----Insert Public SSH Key Here---->"
...
#$action = New-ScheduledTaskAction -Execute 'Powershell.exe' -Argument "java -jar C:\Users\jenkins\Downloads\agent.jar -jnlpUrl http://$jenkinsHost/computer/$NODE_NAME/jenkins-agent.jnlp -auth $pair -secret $SECRET"
$action = New-ScheduledTaskAction -Execute 'Powershell.exe' -Argument "C:\Users\$username\Documents\start-jenkins-agent.ps1"
$delay = New-TimeSpan -Minutes 1  
$taskUser = "$Env:COMPUTERNAME\$username"
echo $taskUser
$trigger = New-ScheduledTaskTrigger -AtLogon -User $taskUser -RandomDelay $delay 
Register-ScheduledTask "Jenkins Agent" -Action $action -User $taskUser -Trigger $trigger
